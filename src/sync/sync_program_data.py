        ...
        for worksheet_name in settings.TRADE_WORKSHEETS:
            ...
            try:
                ...
                if not target_mask.any():
                    print(f"시트 '{worksheet_name}'에 채울 데이터가 없습니다.")
                    continue
                ...
                print(f"KIS API에서 프로그램 수급 데이터 조회 중 (총 {len(all_coros)}개 종목, 청크 단위 병렬 처리)...")
                
                # 10개씩 청크로 나누어 처리하여 DNS 부하 및 TPS 관리
                CHUNK_SIZE = 10
                results = []
                
                with tqdm(total=len(all_coros), desc=f"{worksheet_name} 프로그램 조회") as pbar:
                ...
                # 결과 처리 및 업데이트 목록 생성
                updates = []
                for (code, group), prog_map in zip(grouped_keys, results):
                    if not prog_map:
                        continue
