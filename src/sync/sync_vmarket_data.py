...
def build_date_index_map(data_list: list[dict[str, Any]]) -> dict[str, float]:
...
def update_gsheet_with_calculated_data(
    ...
    kospi200_map: dict[str, float],
    hv_map: dict[str, float],
    vkosdaq_map: dict[str, float] | None = None,
) -> None:
    ...
    try:
        ...
        for ws_name in worksheet_names:
            ...
            all_values, ws = manager.get_all_values(sheet_name, ws_name)
            if not all_values:
                continue
            ...
            try:
                ...
                print("  [Error] '(매수날짜)' 컬럼을 찾을 수 없습니다.")
                continue
...
async def main() -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        _, hv_map_kospi = (
            calculate_historical_volatility(kospi200_data) if kospi200_data else (None, {})
        )
        ...
        _, hv_map_kosdaq = (
            calculate_historical_volatility(kosdaq150_data) if kosdaq150_data else (None, {})
        )
