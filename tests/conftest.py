...
def pytest_runtest_teardown(item: pytest.Item, nextitem: pytest.Item | None) -> Generator[None, None, None]:
    """Restore live settings delegation after every test's fixture teardown."""
    try:
        yield
    finally:
...
def _isolate_production_data_dir(
    ...
    # Derived paths follow DATA_DIR through live delegation.
...
def _isolate_kis_token_state(monkeypatch: pytest.MonkeyPatch, tmp_path_factory: pytest.TempPathFactory) -> None:
    ...
    # model_validator(auction-slot rule)와 충돌한다(실측: 2026-09-20
