...
def test_research_credentials_return_declared_order() -> None:
    from src.api.kis.key_pool import resolve_research_credentials

    env = _research_env()
    creds = resolve_research_credentials(env, slots=("5", "1"))
    assert [c.slot for c in creds] == ["DATA_5", "DATA_1"]
    assert creds[0].app_key == "key5"


def test_research_credentials_forbid_trade_key_collision() -> None:
    import pytest
    ...
    with pytest.raises(ValueError, match="collides"):
        resolve_research_credentials(env, slots=("5",))


def test_research_credentials_forbid_decision_shard_overlap() -> None:
    """실측 회귀: krx-alpha 등 다른 소비자와의 슬롯 충돌을 서면 증명이 아니라
    코드로 직접 막는다 — 결정창 샤드로 이미 배정된 슬롯은 연구용으로 재사용 불가."""
    import pytest

    from src.api.kis.key_pool import resolve_research_credentials

    env = {**_research_env(), "KIS_DECISION_SHARD_SLOTS": "1"}
    with pytest.raises(ValueError, match="overlaps KIS_DECISION_SHARD_SLOTS"):
        resolve_research_credentials(env, slots=("1",))
    creds = resolve_research_credentials(env, slots=("5",))
    assert [c.slot for c in creds] == ["DATA_5"]


def test_research_credentials_reject_malformed_declarations() -> None:
    import pytest
    ...
    with pytest.raises(ValueError, match="not declared"):
        resolve_research_credentials({}, slots=("5",))
    with pytest.raises(ValueError, match="not declared"):
        resolve_research_credentials(env, slots=())
    with pytest.raises(ValueError, match="duplicate research slot"):
        resolve_research_credentials(env, slots=("5", "5"))
    with pytest.raises(ValueError, match="invalid research slot"):
        resolve_research_credentials(env, slots=("0",))
    with pytest.raises(ValueError, match="not in KIS_DATA_SLOTS"):
        resolve_research_credentials(env, slots=("9",))
    missing = dict(env)
    del missing["KIS_DATA_5_APP_SECRET"]
    with pytest.raises(ValueError, match="missing credentials"):
        resolve_research_credentials(missing, slots=("5",))
    dup = dict(env)
