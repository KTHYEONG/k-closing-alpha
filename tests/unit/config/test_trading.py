...
def test_trading_settings_defaults() -> None:
    ...
    assert settings.PAPER_SEED_CAPITAL == 10_000_000
    assert settings.PAPER_ENTRY_SIZING_BUFFER_BP == 0.0


def test_settings_history_paths_follow_data_dir(tmp_path) -> None:
    settings = Settings(
        BASE_DIR=tmp_path,
