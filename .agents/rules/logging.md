...
trigger:
  - on_label: ["logging"]
  - on_file_path_regex: "src/.*(logging|engine|execution|api|pipeline|service|orchestration|daily).*"
  - on_file_path_glob: ["src/**/logging/**/*.py", "src/**/execution/**/*.py", "src/**/daily/**/*.py"]
priority: 9
---
...
- **Strict Credential Redaction:** NEVER log secrets, API keys, private tokens, webhook credentials, or sensitive account information.
...
- **Operational vs. High-Frequency Logs:** Keep high-frequency streaming data (e.g., tick-level fills, raw optimizer trials, detailed memory snapshots) isolated in dedicated log destinations (e.g., specific `.jsonl` or diagnostic artifacts) rather than spamming primary system logs.
- **INFO vs. DEBUG/TRACE:**
