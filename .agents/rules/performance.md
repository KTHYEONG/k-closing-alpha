...
trigger:
  ...
  - on_file_path_regex: "src/.*(backfill|data|ml|processing|daily|strategy|execution|backtest|features|engine|portfolio).*"
  - on_file_path_glob: ["src/**/backfill/**/*.py", "src/**/data/**/*.py", "src/**/ml/**/*.py", "src/**/processing/**/*.py", "src/**/strategy/**/*.py", "src/**/execution/**/*.py", "src/**/backtest/**/*.py", "src/**/features/**/*.py"]
priority: 10
---
...
- **Escalate When Justified:** If a measured hot path cannot be efficiently handled with standard vectorization (e.g., sequential state-dependent recurrence), evaluate JIT compilation, native code, out-of-core engines, or parallelism based on the bottleneck shape. Adopt added complexity only when benchmarks demonstrate meaningful gains.
