...
-**Environment Manager:** This project uses `uv` to manage dependencies and virtual environments. The active virtual environment is designated as the `stock` environment.
...
-**Environment Naming:** All references to virtual environment validation, terminal activation, and self-checks MUST use the name `stock` (or `stock` env) instead of `crypto`.
